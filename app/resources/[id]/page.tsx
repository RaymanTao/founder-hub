...
export default async function ResourceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <Section>
    ...
    {resource.cover ? <div className="mt-8 max-w-4xl overflow-hidden rounded-[1.25rem] border border-[var(--border)] bg-[var(--surface)]"><img src={resource.cover} alt={`${resource.title}封面`} className="max-h-[30rem] w-full object-cover" /></div> : null}
    <div className="mt-10 grid gap-5 md:grid-cols-2">
      ...
      <div className="rounded-[1.25rem] border border-[var(--border)] bg-[var(--surface)] p-6"><p className="text-sm leading-7 text-[var(--secondary)]">{resource.access === "Member" ? "这是会员专属资料。完成会员解锁后，将在这里提供安全下载地址和必要的访问信息。" : "这是免费资料。提交邮箱后即可领取，后续更新会同步通知。"}</p>{resource.accessCode ? <p className="mt-4 rounded-[0.9rem] bg-[rgba(138,106,82,0.08)] px-4 py-3 text-sm text-[var(--secondary)]">百度云盘提取码：<span className="font-semibold text-[var(--foreground)]">{resource.accessCode}</span></p> : null}{resource.access === "Member" ? <Link href="/login" className="mt-6 inline-flex rounded-full bg-[var(--foreground)] px-5 py-3 text-sm font-medium text-white">登录并解锁</Link> : resource.status === "Free" ? <Link href={`/api/resources/download/${resource.id}`} className="mt-6 inline-flex rounded-full bg-[var(--foreground)] px-5 py-3 text-sm font-medium text-white">打开资料</Link> : <p className="mt-6 text-sm text-[var(--muted)]">即将开放</p>}</div>
    </div>
